...
Editor.prototype.getFirstNode = function() {
  let model = this.graph.getModel();
  let modelCells = model.cells;
  let rem = [];

  for (var key in modelCells) {
    if(modelCells[key].value) {
      let nodeName = modelCells[key].edges[0].source.title;

      if(rem.includes(nodeName)) {
        return modelCells[key].edges[0].source;
      }
...
Editor.prototype.getGraphNodes = function() {
  ...
  if(node.edges.length < 2) return graphNodes;

  do {
    node = node.edges[1].target;
    graphNodes.push(node);
  }
  while (node.edges.length === 2);
...
Editor.prototype.getGraphXml = function(ignoreSelection)
{
  ...
  // let secondNode = firstNode.edges[0].target;
  // let thirdNode = secondNode.edges[1].target;
  //
  // let graphNodes = this.getGraphNodes(firstNode);
  // console.log(graphNodes)


  // // console.log(this.graph)
  // // console.log(this.graph.model)
  // // console.log(this.graph.getAllCells())
  // let model = this.graph.getModel();
  // // console.log(model)
  // let modelCells = model.cells;
  // // console.log(modelCells)
  // let onlyCells = [];
  //
  // for (var key in modelCells) {
  //   if(modelCells[key].value) {
  //   // console.log(modelCells[key].edges[0].source.title)
  //   // console.log(modelCells[key].edges[0].target.title)
  //     onlyCells.push(modelCells[key])
  //   }
  // }
