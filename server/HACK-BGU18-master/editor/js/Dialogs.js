...
var FilenameDialog = function(editorUi, filename, buttonText, fn, label, validateFn, content, helpLink, closeOnBtn, cancelFn)
{
	...
	this.init = function()
	{
		...
		if (Graph.fileSupport)
		{
			...
			{
				// IE 10 does not implement pointer-events so it can't have a
				// drop highlight
...
/**
 * Hook for getting the export format. Returns null for the default intermediate
 * XML export format or a function that returns the parameter and value to be
 * used in the request in the form key=value, where value should be URL encoded.
 */
	...
	function refresh()
	{
		...
		function addLayer(index, label, child, defaultParent)
		{
			...
			mxEvent.addListener(ldiv, 'dragstart', function(evt)
			{
				...
				if (mxClient.IS_FF)
				{
					// LATER: Check what triggers a parse as XML on this in FF
					// after drop
