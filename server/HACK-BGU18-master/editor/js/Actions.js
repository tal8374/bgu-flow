...
Actions.prototype.init = function() {
	...
	var isGraphEnabled = function() {
		return Action.prototype.isEnabled.apply(this, arguments)
				&& graph.isEnabled();
	};
	...
	this.addAction('import...', function() {
		...
		window.openFile.setConsumer(mxUtils.bind(this, function(xml, filename) {
			try {
				...
				editor.graph.setSelectionCells(editor.graph
						.importGraphModel(doc.documentElement));
			} catch (e) {
		...
		ui.showDialog(new OpenDialog(this).container, 320, 220, true, true,
				function() {
	...
	this.addAction('runModel', function() {
	...
	this.addAction('editCode', function() {
	...
	this.addAction('redo', function() {
		ui.redo();
	}, null, 'sprite-redo', (!mxClient.IS_WIN) ? Editor.ctrlKey + '+Shift+Z'
			: Editor.ctrlKey + '+Y');
	...
	this
			.addAction(
					...
					function(evt) {
						if (graph.isEnabled()
								&& !graph
										.isCellLocked(graph.getDefaultParent())) {
							...
							try {
								...
								if (cells != null) {
									...
									if (bb != null) {
										var x = Math
												.round(graph
														.snap(graph.popupMenuHandler.triggerX
																/ s - dx));
										var y = Math
												.round(graph
														.snap(graph.popupMenuHandler.triggerY
																/ s - dy));

										graph.cellsMoved(cells, x - bb.x, y
												- bb.y);
									}
								}
							} finally {
			...
			if (parents != null) {
				...
				for (var i = 0; i < parents.length; i++) {
					if (graph.model.contains(parents[i])
							&& (graph.model.isVertex(parents[i]) || graph.model
									.isEdge(parents[i]))) {
	...
	this.put('turn', new Action(mxResources.get('turn') + ' / '
			+ mxResources.get('reverse'), function() {
	...
	this.addAction('lockUnlock',
			function() {
				if (!graph.isSelectionEmpty()) {
					graph.getModel().beginUpdate();
					try {
						var defaultValue = graph.isCellMovable(graph
								.getSelectionCell()) ? 1 : 0;
						graph.toggleCellStyles(mxConstants.STYLE_MOVABLE,
								defaultValue);
						graph.toggleCellStyles(mxConstants.STYLE_RESIZABLE,
								defaultValue);
						graph.toggleCellStyles(mxConstants.STYLE_ROTATABLE,
								defaultValue);
						graph.toggleCellStyles(mxConstants.STYLE_DELETABLE,
								defaultValue);
						graph.toggleCellStyles(mxConstants.STYLE_EDITABLE,
								defaultValue);
	...
	this.addAction('ungroup',
			function() {
				if (graph.getSelectionCount() == 1
						&& graph.getModel().getChildCount(
								graph.getSelectionCell()) == 0) {
	...
	this.addAction('editTooltip...', function() {
		...
		if (graph.isEnabled() && !graph.isSelectionEmpty()) {
			...
			var dlg = new TextareaDialog(ui, mxResources.get('editTooltip')
					+ ':', tooltip, function(newValue) {
												...
												if (link.length > 0) {
													...
													var linkCell = new mxCell(
															...
															'fontColor=#0000EE;fontStyle=4;rounded=1;overflow=hidden;'
																	+ ((icon != null) ? 'shape=label;imageWidth=16;imageHeight=16;spacingLeft=26;align=left;image='
																			+ icon
																			: 'spacing=10;'));
													linkCell.vertex = true;

													graph.setLinkForCell(
															linkCell, link);
													graph.cellSizeUpdated(
															linkCell, true);
	...
	this.addAction('link...', mxUtils.bind(this, function() {
		...
		if (graph.isEnabled()) {
			if (graph.cellEditor.isContentEditing()) {
				var link = graph.getParentByName(graph.getSelectedElement(),
						'A', graph.cellEditor.textarea);
				var oldValue = '';
				...
				ui.showLinkDialog(oldValue, mxResources.get('apply'), mxUtils
			...
			try {
				for (var i = 0; i < cells.length; i++) {
					...
					if (graph.getModel().getChildCount(cell)) {
						...
						if (graph.getModel().isVertex(cell) && state != null
								&& state.text != null && geo != null
								&& graph.isWrapping(cell)) {
							geo = geo.clone();
							geo.height = state.text.boundingBox.height
									/ graph.view.scale;
			...
			try {
				if (state.style['html'] == '1') {
					...
					if (mxUtils.getValue(state.style, 'nl2Br', '1') != '0') {
						// Removes newlines from HTML and converts breaks to
						// newlines
					...
					var label = mxUtils.htmlEntities(graph
							.convertValueToString(state.cell), false);
					...
					graph.cellLabelChanged(state.cell, graph
							.sanitizeHtml(label));
				}
				...
				ui.fireEvent(new mxEventObject('styleChanged', 'keys',
						[ 'html' ], 'values',
						[ (value != null) ? value : '0' ], 'cells', graph
								.getSelectionCells()));
			} finally {
				graph.getModel().endUpdate();
			}
		...
		var dlg = new FilenameDialog(ui, value, mxResources.get('apply'),
				function(newValue) {
					if (newValue != null && newValue.length > 0) {
						...
						+ mxResources.get('rotation') + ' 0-360)');
					...
					mxUtils
							.bind(
									...
									function() {
										...
										var scale = Math.floor(20 * Math.min(cw
												/ fmt.width / ps, ch
												/ fmt.height / ps)) / 20;
										graph.zoomTo(scale);

										if (mxUtils
												.hasScrollbars(graph.container)) {
											var pad = graph.getPagePadding();
											graph.container.scrollTop = pad.y
													* graph.view.scale;
											graph.container.scrollLeft = Math
													.min(
															pad.x
																	* graph.view.scale,
							...
							.bind(
									...
									function() {
										...
										if (mxUtils
												.hasScrollbars(graph.container)) {
											var pad = graph.getPagePadding();
											graph.container.scrollTop = Math
													.min(
															pad.y,
	...
	this.put('customZoom', new Action(mxResources.get('custom') + '...',
			mxUtils.bind(this, function() {
				var dlg = new FilenameDialog(this.editorUi, parseInt(graph
						.getView().getScale() * 100), mxResources.get('apply'),
	...
	this.addAction('pageScale...', mxUtils.bind(this, function() {
		var dlg = new FilenameDialog(this.editorUi,
				parseInt(graph.pageScale * 100), mxResources.get('apply'),
				mxUtils.bind(this, function(newValue) {
					var val = parseInt(newValue);
	...
	action = this.addAction('copyConnect', function() {
		graph.connectionHandler.setCreateTarget(!graph.connectionHandler
				.isCreateTarget());
	...
	this.put('about', new Action(mxResources.get('about') + ' Graph Editor...',
			function() {
				if (!showingAbout) {
					ui.showDialog(new AboutDialog(ui).container, 320, 280,
							true, true, function() {
								showingAbout = false;
							});
	...
	// Font style actions
	var toggleFontStyle = mxUtils.bind(this,
	...
	this.addAction('backgroundColor...',
			function() {
	...
	this.addAction('dashed', function() {
		...
		try {
			...
			graph.setCellStyles(mxConstants.STYLE_DASH_PATTERN, null);
			ui
					.fireEvent(new mxEventObject('styleChanged', 'keys', [
							mxConstants.STYLE_DASHED,
							mxConstants.STYLE_DASH_PATTERN ], 'values', [ '1',
							null ], 'cells', graph.getSelectionCells()));
		} finally {
			graph.getModel().endUpdate();
		}
	});
			...
			ui
					.fireEvent(new mxEventObject('styleChanged', 'keys',
							[ mxConstants.STYLE_ROUNDED,
									mxConstants.STYLE_CURVED ], 'values', [
									'0', '0' ], 'cells', graph
									.getSelectionCells()));
		} finally {
			graph.getModel().endUpdate();
	...
	this.addAction('toggleRounded', function() {
		if (!graph.isSelectionEmpty() && graph.isEnabled()) {
			...
			try {
				...
				var style = (state != null) ? state.style : graph
	...
	this.addAction('collapsible', function() {
		...
		ui.fireEvent(new mxEventObject('styleChanged', 'keys',
				[ 'collapsible' ], 'values', [ value ], 'cells', graph
						.getSelectionCells()));
	});
	this.addAction('editStyle...', mxUtils.bind(this, function() {
		...
		if (cells != null && cells.length > 0) {
			...
			var dlg = new TextareaDialog(this.editorUi, mxResources
					.get('editStyle')
	...
	this.addAction('addWaypoint', function() {
		...
		if (cell != null && graph.getModel().isEdge(cell)) {
			...
			if (handler instanceof mxEdgeHandler) {
				...
				var x = Math.round(graph.snap(graph.popupMenuHandler.triggerX
						/ s - dx));
				var y = Math.round(graph.snap(graph.popupMenuHandler.triggerY
						/ s - dy));
	...
	this.addAction('removeWaypoint', function() {
		...
		if (rmWaypointAction.handler != null) {
			// NOTE: Popupevent handled and action updated in
			// Menus.createPopupMenu
			rmWaypointAction.handler.removePoint(
					rmWaypointAction.handler.state, rmWaypointAction.index);
		}
	});
			...
			.addAction(
					...
					function() {
						if (graph.isEnabled()
								&& !graph
										.isCellLocked(graph.getDefaultParent())) {
							var title = mxResources.get('image') + ' ('
									+ mxResources.get('url') + '):';
							var state = graph.getView().getState(
							...
							var selectionState = graph.cellEditor
									.saveSelection();
											...
											function(newValue, w, h) {
												...
												if (graph.cellEditor
													...
													if (newValue != null
														...
														if (select != null) {
															graph
																	.setSelectionCells(select);
															graph
																	.scrollCellToVisible(select[0]);
														}
													}
												}
											}, graph.cellEditor
	...
	action = this.addAction('layers', mxUtils.bind(this, function() {
		if (this.layersWindow == null) {
			...
			this.layersWindow = new LayersWindow(ui,
					document.body.offsetWidth - 280, 120, 220, 180);
			...
			this.layersWindow.window.setVisible(!this.layersWindow.window
					.isVisible());
		}
	}), null, null, Editor.ctrlKey + '+Shift+L');
	action.setToggleAction(true);
	action.setSelectedCallback(mxUtils.bind(this, function() {
		return this.layersWindow != null
	...
	action = this.addAction('outline', mxUtils.bind(this, function() {
		if (this.outlineWindow == null) {
			...
			this.outlineWindow = new OutlineWindow(ui,
					document.body.offsetWidth - 260, 100, 180, 180);
			...
			this.outlineWindow.window.setVisible(!this.outlineWindow.window
					.isVisible());
		}
