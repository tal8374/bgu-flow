...
Editor = function(chromeless, themes, model, graph, editable) {
	...
	this.graphChangeListener = function(sender, eventObject) {
		var edit = (eventObject != null) ? eventObject.getProperty('edit')
				: null;
	...
	this.graph.getModel().addListener(mxEvent.CHANGE,
			mxUtils.bind(this, function(sender, eventObject) {
...
(function() {
	try {
		...
		while (op.opener != null && typeof op.opener.Editor !== 'undefined'
				...
				// Workaround for possible infinite loop in FF
				// https://drawio.atlassian.net/browse/DS-795
				op.opener != op) {
			op = op.opener;
		}
...
/**
 * Specifies if local storage should be used (eg. on the iPad which has no
 * filesystem)
 */
...
/**
 * Specifies if the canvas should be extended in all directions. Default is
 * true.
 */
...
/**
 * Specifies the order of OK/Cancel buttons in dialogs. Default is true. Cancel
 * first is used on Macs, Windows/Confluence uses cancel last.
 */
...
/**
 * Contains the current modified state of the diagram. This is false for new
 * diagrams and after the diagram was saved.
 */
 ...
 * 
 */
Editor.prototype.editBlankUrl = window.location.protocol + '//'
	...
	if (this.editorWindow != null && !this.editorWindow.closed) {
		...
		if (typeof window.postMessage !== 'undefined'
			...
			if (this.editorWindow == null) {
				mxEvent.addListener(window, 'message', mxUtils.bind(this,
						function(evt) {
							if (evt.data == 'ready'
									&& evt.source == this.editorWindow) {
			...
			this.editorWindow = window.open(this.getEditBlankUrl(p) + '#R'
...
Editor.prototype.readGraphState = function(node) {
	...
	this.graph.gridSize = parseFloat(node.getAttribute('gridSize'))
	...
	if (pw != null && ph != null) {
		this.graph.pageFormat = new mxRectangle(0, 0, parseFloat(pw),
				parseFloat(ph));
	}
...
Editor.prototype.setGraphXml = function(node) {
	if (node != null) {
		...
		if (node.nodeName == 'mxGraphModel') {
			...
			// Workaround for invalid XML output in Firefox 20 due to bug in
			// mxUtils.getXml
...
Editor.prototype.getGraphXml = function(ignoreSelection) {
	...
	if (ignoreSelection) {
		...
		node = this.graph.encodeCells(mxUtils.sortCells(this.graph.model
				.getTopmostCells(this.graph.getSelectionCells())));
	}

	if (this.graph.view.translate.x != 0 || this.graph.view.translate.y != 0) {
		node.setAttribute('dx',
				Math.round(this.graph.view.translate.x * 100) / 100);
		node.setAttribute('dy',
				Math.round(this.graph.view.translate.y * 100) / 100);
	}

	node.setAttribute('grid', (this.graph.isGridEnabled()) ? '1' : '0');
	node.setAttribute('gridSize', this.graph.gridSize);
	node.setAttribute('guides', (this.graph.graphHandler.guidesEnabled) ? '1'
			: '0');
	node.setAttribute('tooltips', (this.graph.tooltipHandler.isEnabled()) ? '1'
			: '0');
	node.setAttribute('connect',
			(this.graph.connectionHandler.isEnabled()) ? '1' : '0');
	node.setAttribute('arrows', (this.graph.connectionArrowsEnabled) ? '1'
			: '0');
...
Editor.prototype.createUndoManager = function() {
	...
	var undoHandler = function(sender, evt) {
		var cand = graph
				.getSelectionCellsForChanges(evt.getProperty('edit').changes);
...
function Dialog(editorUi, elt, w, h, modal, closable, onClose, noScroll) {
	...
	var dh = Math.max(document.body.clientHeight,
			document.documentElement.clientHeight);
	var left = Math
			.max(1, Math.round((document.body.clientWidth - w - 64) / 2));
	...
	// Increments zIndex to put subdialogs and background over existing dialogs
	// and background
	...
	if (closable) {
		...
		mxEvent.addGestureListeners(this.bg, null, null, mxUtils.bind(this,
				function(evt) {
					editorUi.hideDialog(true);
				}));
	}
	...
	this.resizeListener = mxUtils.bind(this, function() {
		dh = Math.max(document.body.clientHeight,
				document.documentElement.clientHeight);
		this.bg.style.height = dh + 'px';
...
PrintDialog.prototype.create = function(editorUi) {
	...
	function preview(print) {
		...
		if (autoOrigin) {
			var pageCount = (onePageCheckBox.checked) ? 1
		...
		var preview = PrintDialog.createPrintPreview(graph, scale, pf, border,
				x0, y0, autoOrigin);
		preview.open();
	...
	var printBtn = mxUtils.button(mxResources
			.get((!PrintDialog.previewEnabled) ? 'ok' : 'print'), function() {
		editorUi.hideDialog();
		preview(true);
	});
 ...
 * Constructs a new print dialog.
 */
PrintDialog.createPrintPreview = function(graph, scale, pf, border, x0, y0,
		autoOrigin) {
...
var PageSetupDialog = function(editorUi) {
	...
	var accessor = PageSetupDialog.addPageFormatPanel(td, 'pagesetupdialog',
			graph.pageFormat);
	...
	function updateBackgroundColor() {
		if (newBackgroundColor == null
			...
			backgroundButton.style.backgroundImage = 'url(\''
	...
	mxEvent.addListener(gridSizeInput, 'change', function() {
		...
		gridSizeInput.value = Math.max(1, (isNaN(value)) ? graph.getGridSize()
				: value);
	});
			...
			.button(
					...
					function() {
						...
						var change = new ChangePageSetup(editorUi,
								newBackgroundColor, newBackgroundImage,
								accessor.get());
						change.ignoreColor = graph.background == newBackgroundColor;

						var oldSrc = (graph.backgroundImage != null) ? graph.backgroundImage.src
								: null;
						var newSrc = (newBackgroundImage != null) ? newBackgroundImage.src
								: null;
 ...
 * 
 */
PageSetupDialog.addPageFormatPanel = function(div, namePostfix, pageFormat,
		pageFormatListener) {
		...
		if (force
			...
			for (var i = 0; i < formats.length; i++) {
				...
				if (customSize) {
					...
					if (pageFormat.width == f.format.width
						...
						detected = true;
					} else if (pageFormat.width == f.format.height
	...
	var update = function() {
		...
		var newPageFormat = new mxRectangle(0, 0, Math
				.floor(parseFloat(widthInput.value) * 100), Math
				.floor(parseFloat(heightInput.value) * 100));

		if (paperSizeSelect.value != 'custom' && landscapeCheckBox.checked) {
			newPageFormat = new mxRectangle(0, 0, newPageFormat.height,
					newPageFormat.width);
		}

		if (newPageFormat.width != currentPageFormat.width
...
(function() {
		...
		if (graph.container != null && !graph.transparentBackground) {
			if (graph.pageVisible) {
				...
				if (this.backgroundPageShape == null) {
					...
					if (firstChild != null) {
						this.backgroundPageShape = this
								.createBackgroundPageShape(bounds);
						this.backgroundPageShape.scale = 1;

						// Shadow filter causes problems in outline window in
						// quirks mode. IE8 standards
						// also has known rendering issues inside mxWindow but
						// not using shadow is worse.
						this.backgroundPageShape.isShadow = !mxClient.IS_QUIRKS;
						this.backgroundPageShape.dialect = mxConstants.DIALECT_STRICTHTML;
						this.backgroundPageShape.init(graph.container);

						// Required for the browser to render the background
						// page in correct order
						firstChild.style.position = 'absolute';
						graph.container.insertBefore(
								this.backgroundPageShape.node, firstChild);
						...
						mxEvent.addListener(this.backgroundPageShape.node,
								'dblclick', mxUtils.bind(this, function(evt) {
									graph.dblClick(evt);
								}));

						// Adds basic listeners for graph event dispatching
						// outside of the
						// container and finishing the handling of a single
						// gesture
						mxEvent.addGestureListeners(
								this.backgroundPageShape.node, mxUtils.bind(
										this, function(evt) {
											graph.fireMouseEvent(
													mxEvent.MOUSE_DOWN,
													new mxMouseEvent(evt));
										}), mxUtils.bind(this, function(evt) {
									// Hides the tooltip if mouse is outside
									// container
	...
	mxGraphView.prototype.validateBackgroundStyles = function() {
		var graph = this.graph;
		var color = (graph.background == null || graph.background == mxConstants.NONE) ? graph.defaultPageBackgroundColor
				: graph.background;
		var gridColor = (color != null && this.gridColor != color.toLowerCase()) ? this.gridColor
				: '#ffffff';
		var image = 'none';
		var position = '';
		...
		if (graph.isGridEnabled()) {
			...
			if (mxClient.IS_SVG) {
				...
				image = unescape(encodeURIComponent(this
						.createSvgGrid(gridColor)));
				image = (window.btoa) ? btoa(image) : Base64
						.encode(image, true);
			...
			position = -Math.round(phase
					- mxUtils.mod(this.translate.x * this.scale - x0, phase))
					+ 'px '
					+ -Math.round(phase
							- mxUtils.mod(this.translate.y * this.scale - y0,
									phase)) + 'px';
		}
	...
	mxGraphView.prototype.createSvgGrid = function(color) {
		...
		for (var i = 1; i < this.gridSteps; i++) {
			var tmp3 = i * tmp;
			d.push('M 0 ' + tmp3 + ' L ' + tmp2 + ' ' + tmp3 + ' M ' + tmp3
					+ ' 0 L ' + tmp3 + ' ' + tmp2);
		}

		// KNOWN: Rounding errors for certain scales (eg. 144%, 121% in Chrome,
		// FF and Safari). Workaround
		// in Chrome is to use 100% for the svg size, but this results in
		// blurred grid for large diagrams.
		var size = tmp2;
		var svg = '<svg width="'
				+ size
				+ '" height="'
	...
	mxGraph.prototype.panGraph = function(dx, dy) {
		...
		if (this.shiftPreview1 != null) {
			...
			var position = -Math.round(phase
					- mxUtils.mod(this.view.translate.x * this.view.scale + dx,
							phase))
					+ 'px '
					+ -Math.round(phase
							- mxUtils.mod(this.view.translate.y
									* this.view.scale + dy, phase)) + 'px';
			canvas.style.backgroundPosition = position;
		}
	};
	...
	mxGraph.prototype.updatePageBreaks = function(visible, width, height) {
		...
		var horizontalCount = (visible) ? Math.ceil(height / bounds.height) - 1
				: 0;
		...
		var drawPageBreaks = mxUtils
				.bind(
						this,
						function(breaks) {
							if (breaks != null) {
								var count = (breaks == this.horizontalPageBreaks) ? horizontalCount
										: verticalCount;

								for (var i = 0; i <= count; i++) {
									var pts = (breaks == this.horizontalPageBreaks) ? [
											new mxPoint(Math.round(bounds2.x),
													Math.round(bounds2.y
															+ (i + 1)
															* bounds.height)),
											new mxPoint(Math.round(right), Math
													.round(bounds2.y + (i + 1)
															* bounds.height)) ]
											: [
													new mxPoint(
									...
									if (breaks[i] != null) {
										...
										var pageBreak = new mxPolyline(pts,
												this.pageBreakColor);
	...
	mxGraphView.prototype.createBackgroundPageShape = function(bounds) {
		return new mxRectangleShape(bounds, '#ffffff',
	...
	mxGraphView.prototype.getBackgroundPageBounds = function() {
		...
		var bounds = new mxRectangle(this.scale * (this.translate.x + x0 * pw),
				this.scale * (this.translate.y + y0 * ph), this.scale * rows
						* pw, this.scale * cols * ph);
	...
	mxGraph.prototype.panGraph = function(dx, dy) {
		...
		if ((this.dialect != mxConstants.DIALECT_SVG && this.view.backgroundPageShape != null)
				&& (!this.useScrollbarsForPanning || !mxUtils
	...
	mxGraphHandler.prototype.isDelayedSelection = function(cell, me) {
		...
		if (!result) {
			...
			while (parent != null) {
				// Inconsistency for unselected parent swimlane is intended for
				// easier moving
				// of stack layouts where the container title section is too far
				// away
	...
	mxGraphHandler.prototype.selectDelayed = function(me) {
		if (!this.graph.popupMenuHandler.isPopupTrigger(me)) {
			...
			if (state != null && me.isSource(state.control)) {
				...
				while (!this.graph.isCellSelected(parent)
						&& model.isVertex(parent)) {
					cell = parent;
